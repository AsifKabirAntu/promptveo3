...
// Interface for our specific simplified prompt data structure
interface SimplifiedPromptData {
  title: string
  category: string
  description: string
  style: string
...
async function ingestSimplifiedPrompts() {
  try {
    const filePath = 'data/generated_prompts_simplified.json'
    console.log(`📖 Reading data from ${filePath}...`)
    
    const fileContent = fs.readFileSync(filePath, 'utf8')
    const prompts = JSON.parse(fileContent) as SimplifiedPromptData[]
...
// Run the script
ingestSimplifiedPrompts().catch(console.error) 