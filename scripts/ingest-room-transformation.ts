...
// Interface for room transformation prompt data structure
interface RoomTransformationPromptData {
  title: string
  category: string
  description: string
  style: string
...
async function ingestRoomTransformationPrompts() {
  try {
    const filePath = 'data/room_transformation_all_20.json'
    console.log(`📖 Reading data from ${filePath}...`)
    
    const fileContent = fs.readFileSync(filePath, 'utf8')
    const prompts = JSON.parse(fileContent) as RoomTransformationPromptData[]
    
    if (!Array.isArray(prompts)) {
      throw new Error('JSON data is not an array')
    }
    
    console.log(`📊 Found ${prompts.length} room transformation prompts to insert`)
    ...
    console.log(`\n🎉 Room transformation data ingestion complete!`)
...
// Run the script
ingestRoomTransformationPrompts().catch(console.error) 